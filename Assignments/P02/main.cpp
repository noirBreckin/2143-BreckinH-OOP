/************************************************************************
 *
 *  Author:           Breckin Hume using starter code from Prof. Terry Griffin
 *  Title:            Singly linked list vector implementation using overloaded operators
 *  Course:           2143 Semester:         Fall 2022
 *
 *  Description:
 *        Uses a singly linked list as the backend for an STL like "vector"
 *        class definition. Also demonstrates overloaded operators.
 *
 *  Usage:
...
public:
  ...
  friend std::ostream &operator<<(std::ostream &stream, const MyVector &vector);
  // Overloaded ofstream operator
  friend ofstream &operator<<(ofstream &ofs, const MyVector &v1) {
    ofs << "( " << v1.size << " )" << endl;
    return ofs;
  }
